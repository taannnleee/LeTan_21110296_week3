...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.5:8080/api/login', { 
        method: 'POST',
        headers: {
      ...
      if (responseData.status === 200) {
        // Giả sử responseData chứa id của người dùng
        ...
        Alert.alert('Error',  'Username or password is incorrect.'+error);
    }
  };
